...
const Checkout: React.FC = () => {
  ...
  // Apply promo code fetch from backend only on "Apply" button click
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/api/bookings`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          experienceId: booking.experience._id, // <--- CRUCIAL!
          date: booking.date,
          time: booking.time,
          qty: booking.qty,
          userEmail: email,
      ...
      navigate("/result", {
        state: { 
          success: !!bk, 
          booking: { ...booking, fullName, email, discount, confirmed: !!bk, id: bk?._id }
        }
      });
    } catch (err) {
      // On error still navigate and show unsuccessful
      navigate("/result", { state: { success: false, booking: { ...booking, fullName, email, discount, confirmed: false } } });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-10 flex flex-col md:flex-row gap-10 items-start">
        ...
        >
          ...
          <div className="flex items-center mt-2">
            <input
              ...
              id="agree"
            />
            <label htmlFor="agree" className="text-xs text-gray-600">I agree to the terms and safety policy</label>
          </div>
          {submitError && (
      ...
      <aside className="w-full md:w-[320px] bg-white rounded-xl p-6 border shadow space-y-3 h-fit mt-8 md:mt-0">
        <div className="text-sm font-medium flex justify-between"><span>Experience</span><span>{booking.experience.title}</span></div>
        <div className="flex justify-between text-sm"><span>Date</span><span>{booking.date}</span></div>
        <div className="flex justify-between text-sm"><span>Time</span><span>{booking.time}</span></div>
        <div className="flex justify-between text-sm"><span>Qty</span><span>{booking.qty}</span></div>
        <div className="flex justify-between text-sm"><span>Subtotal</span><span>₹{booking.subtotal}</span></div>
        <div className="flex justify-between text-sm"><span>Taxes</span><span>₹{booking.taxes}</span></div>
        {!!discount && (
        ...
        <button
          type="button"
          className="w-full mt-5 font-bold py-3 rounded transition bg-yellow-400 text-black hover:bg-yellow-500"
          disabled={submitting}
